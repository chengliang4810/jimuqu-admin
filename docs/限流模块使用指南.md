...
jimuqu:
  ratelimit:
    # 限流类型：DEFAULT-默认CacheService实现，CUSTOM-用户自定义实现
    type: DEFAULT
    # 限流维度：IP-IP限流，USER-用户限流，GLOBAL-全局限流
    dimension: IP
    # 每秒生成令牌数
...
solon:
  data:
    cache:
      type: # 其他支持的缓存类型，如memory、caffeine等
```
...
public class TestController {
    ...
    // 用户限流：每个用户每秒5次请求
    @RateLimit(dimension = RateLimitConfig.RateLimitDimension.USER,
               permitsPerSecond = 5)
    ...
    // 全局限流：所有用户总共每秒20次请求
    @RateLimit(dimension = RateLimitConfig.RateLimitDimension.GLOBAL,
               permitsPerSecond = 20)
    ...
    // 使用滑动窗口算法的IP限流
    @RateLimit(dimension = RateLimitConfig.RateLimitDimension.IP,
               algorithm = RateLimitConfig.RateLimitAlgorithm.SLIDING_WINDOW,
               permitsPerSecond = 8,
               maxBurst = 20)
    ...
    // 使用固定窗口算法的用户限流
    @RateLimit(dimension = RateLimitConfig.RateLimitDimension.USER,
               algorithm = RateLimitConfig.RateLimitAlgorithm.FIXED_WINDOW,
               permitsPerSecond = 3,
               window = 60)
